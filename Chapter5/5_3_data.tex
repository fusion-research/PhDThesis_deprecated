...
In this work we focus our analysis from September to November 2017 in four city: Turin (Italy), Milan (Italy), Berlin (Germany), Vancouver (Canada). %We restrict our analysis to this period since the system was in stationary conditions.
...
To give the intuition of the system, we first provide a characterization of actual usage patterns by current FFCS customers in each city. We focus first on the temporal characteristics.
...
Fig.~\ref{fig:cdf_characterization} reports the Cumulative Distribution Function (CDF) of the rentals duration (top), driving distance (middle) and parking duration (bottom).
